...
def rate_treatment(req: RateRequest):
    try:
        ...
        cur.execute(
            ...
            (req.hospital_id, req.booking_id, req.rating, req.rating, req.rating, req.rating, req.rating, float(req.rating), req.review_text)
        )
        conn.commit()
        conn.close()
...
def get_my_review(booking_id: str):
    try:
        conn = get_db_connection()
        cur = conn.cursor(dictionary=True)
        cur.execute("SELECT overall_rating, written_review FROM reviews WHERE booking_id = %s", (booking_id,))
        review = cur.fetchone()
        conn.close()
        if not review:
