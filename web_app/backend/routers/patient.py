    ...
    try:
        ...
        if req.requires_ambulance:
            ...
            if driver:
                ...
                p_rec = cur.fetchone()
                hName = h_rec['name'] if h_rec else "Hospital"
                pContact = p_rec['contact_number'] if p_rec else "N/A"
                
                # Persist to database to survive Vercel context erasure
                disp_str = f"DISP|{req.name}|{hName}|{pContact}|{req.patient_lat}|{req.patient_lon}"
                cur.execute("INSERT INTO hospital_driver_requests (hospital_id, driver_id, status) VALUES (%s, %s, %s)", 
                             (req.hospital_id, driver['driver_id'], disp_str))
