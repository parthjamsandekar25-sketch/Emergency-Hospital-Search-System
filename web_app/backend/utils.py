...
def send_pwd_reset_otp_email(to_email, name, otp):
...
def send_banned_email(to_email, name, user_type):
    subject = "Account Suspended - Notification"
    body = f"""<html><body>
    <p>Dear {name},</p>
    <p>We regret to inform you that your {user_type} account has been suspended by the System Administrator for violations of our policies.</p>
...
def send_query_reply_email(to_email, name, query_text, reply_text):
    subject = "Response to your Support Query"
    body = f"""<html><body>
    <p>Dear {name},</p>
    <p>The System Administrator has responded to your recent query:</p>
    <blockquote style="border-left: 3px solid #ccc; margin-left: 10px; padding-left: 10px; color: #555;">
        <p><strong>Your Question:</strong> {query_text}</p>
    </blockquote>
    <p><strong>Admin Reply:</strong><br>{reply_text}</p>
    <p>Regards,<br>System Administrator HQ</p>
    </body></html>"""
    msg = MIMEText(body, 'html')
    msg['Subject'] = subject
    msg['From'] = SENDER_EMAIL
