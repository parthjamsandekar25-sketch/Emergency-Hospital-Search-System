...
def send_account_approved_email(to_email, hospital_name, base_url="http://localhost:8000"):
    ...
    body = (
        ...
        f"Your registration request has been verified and approved by the Super Admin.\n"
        f"You can now log in to your account at: {base_url}/static/admin.html\n\n"
...
def send_magic_link_email(to_email, hospital_name, token, base_url="http://localhost:8000"):
    ...
    body = (
        f"Hello {hospital_name},\n\n"
        f"We received a registration request for your hospital on the Hospital Portal.\n"
        f"Did you request this?\n\n"
        f"Click YES if it was you: {yes_link}\n\n"
        f"Click NO to cancel and delete the request: {no_link}\n\n"
        f"Regards,\nSystem Administrator"
    )
