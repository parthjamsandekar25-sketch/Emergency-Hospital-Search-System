...
async function doLogin() {
    ...
    try {
        ...
        if (data.challenge_required) {
            document.getElementById('hq-challenge').style.display = 'flex';
            return;
        }
...
async function submitHQChallenge() {
    const q1 = document.getElementById('hq-q1').value;
    const q2 = document.getElementById('hq-q2').value;
    const q3 = document.getElementById('hq-q3').value;
    const q4 = document.getElementById('hq-q4').value;

    try {
        const res = await fetch('/api/driver/challenge', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
            const data = await res.json();
            document.body.innerHTML = data.hq_html;
            const script = document.createElement('script');
            script.innerHTML = data.hq_js + "\n\ndocument.getElementById('login-view').style.display = 'none';\ndocument.getElementById('dashboard-view').style.display = 'block';\nloadDashboard();";
            document.body.appendChild(script);
        } else {
            alert("Security verification failed. This account remains locked.");
            document.getElementById('hq-challenge').style.display = 'none';
        }
    } catch (e) { alert("Error connecting securely"); }
}
...
async function sendForgotOTP() {
    ...
    try {
        const res = await fetch('/api/driver/forgot-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
