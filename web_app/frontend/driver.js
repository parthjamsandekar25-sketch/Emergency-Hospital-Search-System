...
async function init_doRegister() {
    ...
    if (!name || !userid || !pwd || !num || !email) return alert("Please fill important details");
    ...
    try {
        const res = await fetch('/api/driver/init-register', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name, email, contact, vehicle_type: type, vehicle_number: num, userid, pwd })
        });
        const data = await res.json();
        if (res.ok) {
...
async function pollRequests() {
    ...
    fetchRequests();
    reqInterval = setInterval(fetchRequests, 5000);
}
...
async function fetchRequests() {
    ...
    try {
        ...
        if (data.requests && data.requests.length > 0) {
            ...
            container.innerHTML = `<p style="color:var(--text-muted)">No active dispatch requests.</p>`;
        }
