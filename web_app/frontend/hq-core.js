...
async function loadUsers() {
    try {
        ...
        let html = '';
        const types = ['hospitals', 'patients', 'drivers'];

        types.forEach(pt => {
            html += `<h3 style="color:var(--primary); text-transform:capitalize; margin:20px 0 10px;">${pt}</h3>`;
            if (data.users[pt].length === 0) {
                html += `<p style="color:#666;">No ${pt} found.</p>`;
            } else {
                data.users[pt].forEach(u => {
                    const isBanned = u.is_banned ? '<span style="color:var(--danger); font-weight:bold; margin-left:10px;">[BANNED]</span>' : '';
                    const banBtn = u.is_banned ? '' : `<button class="btn-reject" onclick="banUser('${pt.slice(0, -1)}', ${u.id})">BAN</button>`;
                    html += `
                        <div class="list-item">
                            <div class="list-details">
