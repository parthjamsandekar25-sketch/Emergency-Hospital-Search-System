...
async function init_doRegister() {
    ...
    try {
        ...
        if (res.ok) {
            alert("Verification link sent to your email. Click 'YES' in the email to submit your registration to Super Admin.");
            toggleAuth('login');
        } else {
            const data = await res.json();
...
async function updateHospitalAccount() {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            alert("Account update request sent to Super Admin for approval!");
