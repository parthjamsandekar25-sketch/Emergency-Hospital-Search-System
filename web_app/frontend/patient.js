...
function openQueryModal() {
    if (!currentUser) return alert("Please login first to contact support.");
...
async function submitSupportQuery() {
    ...
    try {
        const res = await fetch('/api/patient/submit-query', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(req)
        });
        if (res.ok) {
            alert("Query sent to Super Admin HQ successfully!");
