...
async function submitRating() {
    ...
    const rating = parseInt(document.getElementById('rate-stars').value);
    ...
    try {
        const res = await fetch('/api/patient/rate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ booking_id, hospital_id, rating, review_text })
        });
        ...
        if (res.ok) {
            ...
            c.innerHTML = `
                <div style="padding:1rem; background: rgba(0, 119, 182, 0.05); border-radius: 8px;">
                    <div style="font-size:1.2rem; font-weight:bold; color:var(--color-patient); margin-bottom:0.5rem;">
