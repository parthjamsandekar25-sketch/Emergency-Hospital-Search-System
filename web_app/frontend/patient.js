        ...
        if (data.results && data.results.length > 0) {
            ...
            data.results.forEach(h => {
                ...
                <div class="h-card">
                    ...
                    <button class="btn" style="padding: 0.5rem 1rem; margin-left:0.5rem; background:#e2e8f0;" onclick="viewReviews(${h.hospital_id}, '${h.name.replace(/'/g, "\\'")}')">View Reviews</button>
                </div>`;
            });
...
async function bookBed(hospital_id, treatment_id) {
    ...
    try {
        ...
        const res = await fetch('/api/patient/book-bed', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: currentUser.email, name: patientName, hospital_id, treatment_id })
        });
        const data = await res.json();
        if (res.ok) {
            let msg = "Bed officially booked!\n";
            if (data.driver) {
                msg += `An ambulance has been assigned to you.\nDriver: ${data.driver.name}\nVehicle: ${data.driver.vehicle_number} (${data.driver.vehicle_type})\nContact: ${data.driver.contact_number}`;
            } else {
                msg += "Currently no ambulances available at this hospital.";
            }
            alert(msg);
            loadMedicalHistory();
...
function openRateModal(booking_id, hospital_id, hospital_name) {
    ...
    document.getElementById('rate-doctor').value = '5';
    document.getElementById('rate-hygiene').value = '5';
    document.getElementById('rate-staff').value = '5';
    document.getElementById('rate-facilities').value = '5';
    document.getElementById('rate-speed').value = '5';
...
async function submitRating() {
    ...
    const doctor_care = parseInt(document.getElementById('rate-doctor').value);
    const hygiene = parseInt(document.getElementById('rate-hygiene').value);
    const staff_behavior = parseInt(document.getElementById('rate-staff').value);
    const facilities = parseInt(document.getElementById('rate-facilities').value);
    const speed_of_service = parseInt(document.getElementById('rate-speed').value);
